...
class AbsensiController extends Controller
{
    public function store(Request $request)
    {
      if (Auth::user()->role == 'siswa' || 'guru' || 'ketua_eskul'){
        ...
        $double_absent_check = Absensi::where('mac_address', $request->mac_address)->where('name', $request->name)->where('created_at', 'like', '%'. $now .'%')->Exists();
        }
        ...
        $request->validate([
            ...
            'mac_address' => ['nullable', 'required_if:role, siswa', 'string']
        ]);
